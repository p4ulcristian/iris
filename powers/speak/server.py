...
Uses Chatterbox for text-to-speech synthesis.

Endpoints:
  GET  /health  - Health check
  POST /speak   - Speak text (queued)
  POST /stop    - Stop current playback
  POST /mute    - Mute TTS (ignore requests)
...
try:
    ...
    class TextToSpeech:
        """Dummy TTS when deps not installed"""
        sample_rate = 24000
...
def write_queue_state():
    """Write current queue state to file for bubble display."""
...
def queue_worker():
    ...
    while True:
        try:
            ...
            current_message = text.replace("So, ", "", 1) if text.startswith("So, ") else text
...
def watchdog():
    """Monitor queue worker for hangs."""
...
def init_models():
    """Initialize TTS model and start worker threads."""
    ...
    logger.info(f"Watchdog started (timeout: {WATCHDOG_TIMEOUT}s)")
    ...
    if TTS_AVAILABLE:
        ...
        try:
            audio = tts_model.synthesize("Iris has been started.")
            if audio.size > 0:
...
def speak():
    ...
    # Prepend filler word for model warmup
...
if __name__ == '__main__':
    run_server(app, PORT, init_fn=init_models, logger=logger)
