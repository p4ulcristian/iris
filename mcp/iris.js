...
// Load MCP tool config from Iris state.json
function loadMcpToolConfig() {
  const statePath = `${homedir()}/.local/share/iris/state.json`;
  try {
    if (existsSync(statePath)) {
      const state = JSON.parse(readFileSync(statePath, "utf-8"));
      return state.settings?.mcpTools || {};
    }
  } catch (e) {}
  return {};
}

const mcpTools = loadMcpToolConfig();
const enabled = (category) => mcpTools[category] !== false;
...
// =============================================================================
// Code (Visual alternatives to Claude's built-in tools)
// =============================================================================
