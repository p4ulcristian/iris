...
```
[User] → [brain/ (STT/TTS)] → [Electron App] → [Gods via Zellij]
              ↑                                      ↓
              └────────────── responses ─────────────┘
```
...
**Gods** are Claude instances in Zellij sessions (persistent terminal multiplexer), rendered via xterm.js. All gods are equal. Each is named from the Greek pantheon (Zeus, Apollo, Artemis, Athena, Hermes, Hades, Poseidon, Hera, Ares, Hephaestus, Aphrodite, Dionysus, Demeter).
...
1. **Summon**: User presses Ctrl+N or clicks [+]
   - Server spawns Zellij session: `zellij attach iris-<name> -b` + `zellij -s iris-<name> run -i -- claude "<prompt>"`
   - React adds god entity to tile

2. **Attach**: When entity renders
   - Bun.Terminal spawns: `zellij attach iris-<name>`
...
4. **Banish**: User presses Ctrl+K or clicks X
   ...
   - `god:kill` event runs `zellij kill-session iris-<name>`

## Session Persistence

Gods use **Zellij** for persistence:
